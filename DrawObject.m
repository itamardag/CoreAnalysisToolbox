...
%         qinsFile = 'Qins-18MII_6.mat'; path = 'E:\Archeology Lab\positioning francesco\';
    ...
    if(withWalls)
        x = [bounds.fullThickness(1) bounds.fullThickness(2)];
        y = [bounds.fullWidth(1) bounds.fullWidth(2)];
        z = [bounds.fullLength(1) bounds.fullLength(2)];
        yBlanks = [bounds.blanksWidth(1) bounds.blanksWidth(2)];
        ...
        surf(x, [yBlanks(1) yBlanks(1)], [z(1) z(1); z(2) z(2)]);
        surf(x, [yBlanks(2) yBlanks(2)], [z(1) z(1); z(2) z(2)]);
        view(-90, 0);
        %%%%
