function [ var ] = var_comb_length( var1,scar_len_min,scar_len_max )%size_lim_low,size_lim_high)
...
% for i=find(var1.core_size<=size_lim_high & var1.core_size>=size_lim_low)'
    var.ang=var1.ang;%=[var.ang;var1.ang{i}];
    var.var=var1.var;%=[var.var;var1.var{i}];
    var.angs=var1.angs;%=[var.angs;var1.angs{i}];
    var.vars=var1.vars;%=[var.vars;var1.vars{i}];
    var.path=var1.path;%=[var.path;var1.path{i}];
    var.name=var1.name;%=[var.name;var1.name{i}];
    var.plat=var1.plat;%=[var.plat;var1.plat{i}];
    var.blanks=var1.blanks;%=[var.blanks;var1.blanks{i}];
    var.dif_pace=var1.dif_pace;%=[var.dif_pace,var1.dif_pace{i}];
    var.dev_pace=var1.dev_pace;%=[var.dev_pace,var1.dev_pace{i}];
    var.scar_number=var1.scar_number;%=[var.scar_number,var1.scar_number(i)];
    var.data=var1.data;%=[var.data,var1.data{i}];
    var.core_size=var1.core_size;%=[var.core_size;var1.core_size(i)];
    var.p_curv=var1.p_curv;
    var.p_L_var=var1.p_L_var;
    var.p_ang_var=var1.p_ang_var;
    var.p_curv_avg=var1.p_curv_avg;
...
for i=1:size(var1.ang,1)
    l_scar_ind=find(sum(~isnan(var.angs{i}))>=scar_len_min & ...
        sum(~isnan(var.angs{i}))<=scar_len_max); %l_scar_ind are the indexes of scars within the length span
